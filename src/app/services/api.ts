...
export const isDevTokenValid = (token?: string): boolean => {
  if (!token) return false;
  const t = token.trim();
  return (
    t === ADMIN_ACCESS_PASSWORD ||
    t.toLowerCase() === 'admin' ||
    t.toLowerCase() === 'developer' ||
    t.toLowerCase() === 'dev' ||
    t.toUpperCase().startsWith('DEV-')
  );
};
...
function publicUser(u: DBUser): User {
 return {
  ...
  location: u.location,
  age: u.age,
  serviceWidget: u.serviceWidget,
  likesCount: u.likesCount,
  mutualCount: u.mutualCount,
  followingCount: u.followingCount,
  followersCount: u.followersCount,
  unreadBadge: u.unreadBadge,
  isLingyuBaby: u.isLingyuBaby,
  isCreator: u.isCreator || u.isLingyuBaby || u.role === 'admin' || !!u.creatorId,
  creatorId: u.creatorId || (u.isCreator ? `CR-${u.id.slice(-4).toUpperCase()}` : undefined),
  bio: u.bio,
  creatorTitle: u.creatorTitle,
  createdAt: u.createdAt,
 } as User;
}
 ...
 auth: {
  register: (payload: {
   ...
   (async () => {
    ...
    const { password, isLingyuBaby, adminPassword, creatorId, avatarColor, avatarUrl, avatarEmoji } = payload;
    if (!douyin || douyin.length < 2) throw new ApiError('抖音用户名至少需要2个字符');
    ...
    const cleanCreatorId = (creatorId || adminPassword || '').trim();
    const hasDevAccess = isDevTokenValid(cleanCreatorId);
    const isCreatorStatus = Boolean(cleanCreatorId) || Boolean(isLingyuBaby) || hasDevAccess;
  ...
  updateProfile: (payload: {
   ...
   resolve<{ message: string; user: User; myNation: Nation | null }>(() => {
    ...
    if (payload.avatarColor !== undefined) {
     updates.avatarColor = payload.avatarColor;
    }
    if (payload.avatarEmoji !== undefined) {
    ...
    if (payload.serviceWidget !== undefined) {
     updates.serviceWidget = payload.serviceWidget;
    }
    if (payload.bio !== undefined) {
     updates.bio = payload.bio.trim();
    }
    if (payload.creatorId !== undefined) {
     updates.creatorId = payload.creatorId.trim();
     if (payload.creatorId.trim()) {
      updates.isCreator = true;
     }
    }
    if (payload.isCreator !== undefined) {
     updates.isCreator = payload.isCreator;
    }
    if (payload.creatorTitle !== undefined) {
     updates.creatorTitle = payload.creatorTitle.trim();
    }
  ...
  create: (payload: Partial<Nation>) =>
   resolve<{ message: string; nation: Nation }>(() => {
    ...
    db.createNotification({
     ...
     type: 'system',
     title: '粉陆开创典仪顺利完成',
     content: `恭喜领主【${user.username}】！您开创的【${newNation.name}】正式成立！执政党确立为【${chosenRulingPartyTitle}】（${chosenIdeologyName}），初始民意支持率为 ${rulingSupport}%，其余在野党席位已由民意随机配额。`,
    ...
    return { message: `粉陆【${newNation.name}】开创成功！执政党确立为【${chosenRulingPartyTitle}】（支持率 100%）`, nation: enrichNation(newNation) };
   }),
