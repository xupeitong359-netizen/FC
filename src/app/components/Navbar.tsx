...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const brandTitle = isMapMode ? '战略地图' : isResearchMode ? '国防科研' : '创联世界';
  const displayName = user?.douyinName || user?.username || '领主';

  return (
    <>
      {/* Pull-Down Handle when Top Bar is Collapsed */}
      {isTopCollapsed && (
      ...
      >
        <div className="w-full max-w-7xl mx-auto px-2 sm:px-4 h-11 sm:h-12 flex items-center justify-between gap-2">
          {/* Left: Raw Brand Trademark + Title */}
          <div
            id="nav-brand-logo"
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            <div className="hidden lg:flex items-center gap-1 px-2.5 py-1 text-slate-400">
          ...
          {/* Right: Independent Notification Bell and User Avatar */}
          <div className="flex items-center gap-2 sm:gap-2.5 flex-shrink-0 ml-auto relative">
            {/* Bug Feedback (2-character pure text: 反馈) */}
            {onOpenBugFeedback && (
              <button
                type="button"
            ...
            {/* Map Construction Action (2-character pure text: 筑造) */}
            {isMapMode && onOpenConstruction && (
              <button
                id="map-top-construction-btn"
                type="button"
                onClick={onOpenConstruction}
                className="px-2.5 py-1 bg-amber-500 hover:bg-amber-600 text-white text-xs font-bold rounded-lg shadow-xs transition-colors flex items-center gap-1 cursor-pointer"
              >
                <span>筑造</span>
              </button>
            )}
            ...
            {isAuthenticated ? (
              <div className="relative" ref={dropdownRef}>
                ...
                {/* Dropdown Menu (2-character pure text items) */}
                {profileDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-48 p-1.5 bg-white border border-slate-200/90 rounded-xl shadow-lg z-50 text-slate-700 animate-fadeIn">
                    <div className="px-2.5 py-2 border-b border-slate-100 mb-1">
                      ...
                      {myNation && (
                        <p className="text-[11px] text-slate-500 truncate mt-1">
                          粉陆：<span className="text-slate-800 font-medium">{myNation.name}</span>
                        </p>
                      )}
                    </div>
                    ...
                    <button
                      ...
                      className="w-full px-2.5 py-1.5 text-xs font-bold text-slate-700 hover:text-indigo-600 hover:bg-slate-50 rounded-lg transition-colors flex items-center gap-2 cursor-pointer text-left"
                    >
                    ...
                    {isAdmin && (
                      <button
                        ...
                        className="w-full px-2.5 py-1.5 text-xs font-bold text-slate-700 hover:bg-amber-50 hover:text-amber-700 rounded-lg transition-colors flex items-center gap-2 cursor-pointer text-left"
                      >
                    ...
                    <button
                      ...
                      className="w-full px-2.5 py-1.5 text-xs font-bold text-rose-600 hover:bg-rose-50 rounded-lg transition-colors flex items-center gap-2 cursor-pointer text-left"
                    >
              ...
              <div className="flex items-center gap-1.5 sm:gap-2">
                <button
                  ...
                  className="px-2.5 py-1 text-xs font-bold text-slate-600 hover:text-slate-900 transition-colors cursor-pointer"
                >
                  登录
                </button>
                <button
                  ...
                  className="px-3 py-1 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-lg shadow-xs transition-colors cursor-pointer"
                >
                  注册
                </button>
              </div>
            ...
            <NotificationPopover
              ...
              onOpenMessagesView={() => setActiveTab('messages')}
            />
          </div>
        </div>

        {/* Pull-Up Collapse Handle at Bottom of Navbar */}
